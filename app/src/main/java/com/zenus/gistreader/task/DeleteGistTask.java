...
public class DeleteGistTask extends AsyncTask<String, String, Void> {
    private DeleteGistListener mStateListener;
    private boolean callbackReceived;
    private static final String t = DeleteGistTask.class.getName();
    private String[] msg = null;

    public DeleteGistTask(){
    ...
    protected Void doInBackground(String... values) {
        ...
        GistApplication.getInstance().getApiController().deleteGist(gistId, new Callback() {
            @Override
            ...
            public Object after(Object... params) {
                ...
                new GistController().deleteGist(gistId);
            ...
            public void error(Object... params) {
                try {
                    if (params[0] instanceof NoConnectionError) {
                        ...
                        DeleteGistTask.this.msg = new String[]{GistApplication.getContext().getString(R.string.no_connection_to_host)};
                    } else {
                        DeleteGistTask.this.msg = new String[]{GistApplication.getContext().getString(R.string.server_error)};
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    DeleteGistTask.this.msg = new String[]{e.getMessage()};
                }
                new CommandController().addCommand(gistId, Commands.DELETE_GIST);
                callbackReceived = true;
            }
        });
    ...
    protected void onPostExecute(Void v) {
        synchronized (this) {
            if (mStateListener != null) {
                mStateListener.gistDeleteComplete();
            }
        }
    }

    public void setTaskListener(DeleteGistListener sl) {
        synchronized (this) {
            mStateListener = sl;
        }
    }
